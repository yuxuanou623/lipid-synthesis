...
sys.path.append(path.join('/home/trin4156/anaconda3/envs/dogae_py3.7_pt1.4/share/RDKit/Contrib/', 'SA_Score'))
import sascorer
...
class PropertyEvaluator:
    ...
    #     return np.array(out)
    def evaluate_molecules(self, list_of_smiles: typing.List[str], round: int):
        out = []
        unseen_mol = []
        canon_smi_list = []
        for smi in list_of_smiles:
            canon_smi = rdkit_general_ops.canconicalize(smi)
            canon_smi_list.append(canon_smi)
            if canon_smi not in self.seen_molecules:
                unseen_mol.append(canon_smi)
        print("unseen_mol", len(unseen_mol))
        if round == -1:
            import pickle

            # Define the path to your pickle file
            file_path = '/mnt/data/synthesis-dags/syn_dags/script_utils/data_2_small_numbers_list.pkl'

            # Open the file in binary read mode
            with open(file_path, 'rb') as file:
                # Load the data from the file
                value_list= pickle.load(file)
            print("round", round)
            print("loaded value_list", len(value_list))
        else:
            value_list = self.property_calculator(unseen_mol)

        for canon_smi, value in zip(unseen_mol,value_list):
            
            self.seen_molecules[canon_smi] = value
        for canon_smi in canon_smi_list:
            out.append(self.seen_molecules[canon_smi])
        return np.array(out)
...
def hela(smile_list):
    ...
    url = "http://localhost:1199/calculate"
    headers = {'Content-Type': 'application/json'}
    batch_size = 500
    total_numbers_list = []
    def batch_smiles(smiles, batch_size):
        """Yield successive batch-sized chunks from smiles."""
        for i in range(0, len(smiles), batch_size):
            yield smiles[i:i + batch_size]

    # Process each batch
    for smiles_batch in batch_smiles(smile_list, batch_size):
        print("Processing batch with size:", len(smiles_batch))  # Debugging line
        data = {"smiles_list": smiles_batch}
        ...
        if response.status_code == 200:
            ...
            numbers = re.findall(r"[-+]?[0-9]*\.?[0-9]+", response.text)  # Make sure to use response.text
            # print("numbers", len(numbers))
            
            # Convert strings to floats and extend the total list
            total_numbers_list.extend([float(num) for num in numbers])
            # print("total_numbers_list", len(total_numbers_list))
        else:
            print("Failed to fetch data for batch:", response.status_code)
    ...
    #     pickle.dump(total_numbers_list, file)
   
    return total_numbers_list
...
def get_task(name_of_task: str):
    ...
    elif name_of_task == 'hela_transfection':
        return PropertyEvaluator(hela)
    else:
