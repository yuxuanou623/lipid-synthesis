...
import sys
sys.path.append(os.path.abspath('/scratch/yo279/Lipid_reaction_dataset_creation/synthesis-dags/submodules/GNN'))
...
class SynthesisTree:
    def __init__(self, tree: nx.DiGraph,
        ...
        if order_for_construction is None:
            order_for_construction = self.tree_to_seq(tree, self.root_smi,self.root_template ,rng)
    ...
    def tuple_tree_to_nx(tuple_tree):
        # the original tuple_tree's node is (smiles,), now is (smiles,template)
        graph = nx.DiGraph()
        final_product_node = (tuple_tree[0],tuple_tree[2])
        graph.add_node(final_product_node)

        def recursive_builder(child_node, list_to_build_out):
            for tt in list_to_build_out:
                parent_node = (tt[0],tt[2])
    ...
    def max_distance_from_root(self):
        print("self.root_template",self.root_template)
        return max(self._longest_path_backwards(self.tree, (self.root_smi,self.root_template)).values())
    ...
    def get_features_required_for_training_model(self,
        ...
        for node in self.order_for_construction:
            ...
            if is_base_reactant:
                ...
                dags_id_at_index.append(len(dags_at_construction_stages)-1)
                if is_final:
                    sequence_action_kinds.append(TEMPLATE_STEP_VAL)
                    sequence_choices.append(int(node[1]))
                ...
                for i, (edge_src, edge_dest) in enumerate(edges_to_add):
                    ...
                    is_last_edge = (i == len(edges_to_add) - 1)
                    if is_last_edge:
                        sequence_action_kinds.append(TEMPLATE_STEP_VAL)
                        sequence_choices.append(int(node[1]))
                        dags_id_at_index.append(len(dags_at_construction_stages)-1)
    ...
    @staticmethod
    def tree_to_seq(tree, root_smi, root_template, rng: np.random.RandomState):
        ...
        lengths = SynthesisTree._longest_path_backwards(tree, (root_smi,root_template))
...
class CollateWithLargestFirstReordering:
    def __init__(self, all_reactants: list,cache_path:str):
        ...
        if os.path.exists(self.cache_path):
            ...
            graphs = []
            for smi in tqdm.tqdm(self.base_mol_to_idx_dict.keys(), desc="Building reactant features"):
                print(smi)
                graphs.append(smiles_to_feats.DEFAULT_SMILES_FEATURIZER.smi_to_feats(smi))
            # Assuming the method to concatenate graphs is correct, adjust if your graph object uses a different method.
            self.reactant_graphs = graphs[0].concatenate(graphs)
           
            with open(self.cache_path, 'wb') as file:
                pickle.dump(self.reactant_graphs, file)
    ...
    def _get_seq_for_each_data_item(self, list_of_tuple_trees):
        ...
        for tt in list_of_tuple_trees:
            ...
            syn_trees.append(SynthesisTree(nx_graph, tt[0], tt[2]))
